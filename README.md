...
3、对于映射目录，主机目录可以任选，容器目录务必统一，这是为了能完美符合脚本的命令。此外主机目录中部份文件可能因为docker环境原因或网络原因难以安装，**(也可以在此github上下载bin.zip和BDinfoCli.0.7.3.zip，解压后拉入nas)**建议在vps上运行脚本，在vps上根据容器目录下载文件，拷贝到对应被映射主机目录中进行使用。所有被映射的主机目录建议给完全访问权限
...
## vps下载部分文件  **(也可以在此github上下载bin.zip和BDinfoCli.0.7.3.zip，解压后拉入nas)**
### 1、vps上执行
